...
fn main() {
    ...
    let tests: [(&str, TestFn); 23] = [
        ...
        (
            "pointer scan: Person generic count",
            test_person_generic_count,
        ),
        (
            "pointer scan: Counter witness count",
            test_counter_witness_count,
        ),
        (
            "pointer scan: Counter generic count",
            test_counter_generic_count,
        ),
        (
            "pointer scan: String has zero generic params",
            test_string_generic_count,
        ),
        (
            "pointer scan: Array<Int32> generic count",
            test_array_generic_count,
        ),
        (
        ...
        (
            "pointer scan: Array kind is generic instantiation",
            test_array_kind,
        ),
        (
            "pointer scan: Direction kind is enum",
            test_direction_pointer_kind,
        ),
        ("pointer scan: Shape metadata id", test_shape_pointer_scan),
        (
            "name scan: N1LayoutStruct field count",
            test_n1_struct_field_count,
        ),
        (
            "name scan: N1LayoutStruct field offsets",
            test_n1_struct_field_offsets,
        ),
        ...
        (
            "name scan: unknown type remains unknown",
            test_unknown_type_name,
        ),
        (
            "discovery path: discovered Layout type introspects",
            test_discovered_layout_type,
        ),
        (
            "registry path: metadata registry exposes bridged types",
            test_registry_entries,
        ),
        (
            "registry path: generic metadata is present",
            test_generic_registry_entries,
        ),
        (
            "registry path: Counter protocol linkage preserved",
            test_counter_protocol_linkage,
        ),
    ];
...
fn test_array_opaque_generic_count(
...
fn test_dictionary_ref_generic_count(
    contract: &RuntimeContract,
) -> Result<bool, RuntimeContractError> {
...
fn test_generic_registry_entries(contract: &RuntimeContract) -> Result<bool, RuntimeContractError> {
    ...
    Ok(entries
        .iter()
        .any(|entry| entry.metadata_id == 1001 && entry.name == "ContractGenericBox<Int32>"))
}
