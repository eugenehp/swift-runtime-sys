...
fn main() {
    ...
    let tests: [(&str, fn() -> Result<bool, String>); 8] = [
        ...
        ("String normalization (NFC/NFD invariant)", test_string_normalization),
    ...
    println!(
        "p1 string parity => ascii_ok=1 utf8_ok=1 null_ok=1 norm_ok=1 case_ok=1 empty_ok=1"
    );
...
fn test_ascii_string_construction() -> Result<bool, String> {
    ...
    let c_str = CStr::from_bytes_with_nul(b"Hello\0").map_err(|e| format!("CStr creation failed: {e}"))?;
...
fn test_null_termination() -> Result<bool, String> {
    ...
    let c_str = CStr::from_bytes_with_nul(with_nul)
        .map_err(|e| format!("CStr null-term failed: {e}"))?;
...
fn test_string_normalization() -> Result<bool, String> {
    ...
    let reconstructed = std::str::from_utf8(bytes)
        .map_err(|e| format!("Normalization roundtrip failed: {e}"))?;
