...
echo "=== Building C.2 Bridge & Thunks ==="
cd "$WORKSPACE"
...
echo ""
echo "=== Phase C.2: Advanced Existentials & Generic Constraints ==="
echo ""

echo "Building C.2 probe (debug)..."
cargo build --example runtime_c2_existentials_probe 2>&1 | grep -E "Compiling|Finished" | head -3

echo "Running C.2 probe (debug)..."
DEBUG_OUTPUT=$(cargo run --example runtime_c2_existentials_probe 2>&1 | tee /tmp/c2_debug.log)
...
echo ""
echo "Building C.2 probe (release)..."
cargo build --example runtime_c2_existentials_probe --release 2>&1 | grep -E "Compiling|Finished" | head -3

echo "Running C.2 probe (release)..."
RELEASE_OUTPUT=$(cargo run --example runtime_c2_existentials_probe --release 2>&1 | tee /tmp/c2_release.log)
...

{
  ...
  echo '  "test_type": "advanced_existentials",'
  ...
  echo "}"
} > "$ARTIFACT_DIR/c2-existentials-gate.json"

{
  echo "# Phase C.2: Advanced Existentials Gate Results"
  echo ""
  ...
  echo "### Debug"
  echo '```'
  cat /tmp/c2_debug.log
  echo '```'
  echo ""
  echo "### Release"
  echo '```'
  cat /tmp/c2_release.log
  echo '```'
} > "$ARTIFACT_DIR/c2-existentials-gate.md"

echo ""
echo "Wrote $ARTIFACT_DIR/c2-existentials-gate.json"
echo "Wrote $ARTIFACT_DIR/c2-existentials-gate.md"

if echo "$DEBUG_STATUS" | grep -q "ALL TESTS PASSED" && echo "$RELEASE_STATUS" | grep -q "ALL TESTS PASSED"; then
  echo ""
  echo "C.2 existentials gate PASSED"
  exit 0
else
  echo ""
  echo "C.2 existentials gate FAILED"
  exit 1
fi
