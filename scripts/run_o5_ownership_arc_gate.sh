...
OUT_DIR="$ROOT/target/runtime-probe/o5-ownership-arc"
FIXTURE_DIR="$ROOT/target/runtime-probe/resilient-fixtures"
MD_OUT="$OUT_DIR/o5-ownership-arc-summary.md"
JSON_OUT="$OUT_DIR/o5-ownership-arc-summary.json"
...
cargo build --example runtime_o5_ownership_arc_probe >/dev/null
cargo build --release --example runtime_o5_ownership_arc_probe >/dev/null

DEBUG_LOG="$OUT_DIR/o5-ownership-arc-debug.log"
RELEASE_LOG="$OUT_DIR/o5-ownership-arc-release.log"

DYLD_LIBRARY_PATH="$FIXTURE_DIR:." "$ROOT/target/debug/examples/runtime_o5_ownership_arc_probe" | tee "$DEBUG_LOG"
DYLD_LIBRARY_PATH="$FIXTURE_DIR:." "$ROOT/target/release/examples/runtime_o5_ownership_arc_probe" | tee "$RELEASE_LOG"
...
if [[ -z "$debug_passed" || -z "$release_passed" ]]; then
  echo "Missing summary output from O.5-wave probe" >&2
  exit 1
fi
...
cat > "$JSON_OUT" <<EOF
{
  "gate": "o5_ownership_arc_coverage",
  "status": "$status",
...
cat > "$MD_OUT" <<EOF
# O.5 Wave: O.6 Ownership Convention + O.7 ObjC Bridge ARC Gate
...
if [[ "$status" != "PASS" ]]; then
  echo "O.5-wave ownership+ARC gate FAILED" >&2
  exit 1
fi

echo "O.5-wave ownership+ARC gate PASSED"
