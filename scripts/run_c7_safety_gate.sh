...
echo "=== Building C.7 Bridge & Thunks ==="
...
echo ""
echo "=== Phase C.7: Runtime Safety Guardrails ==="
echo ""

echo "Building C.7 probe (debug)..."
cargo build --example runtime_c7_safety_probe >/dev/null

echo "Running C.7 probe (debug)..."
DEBUG_LOG="$OUT_DIR/c7-safety-debug.log"
DYLD_LIBRARY_PATH="$FIXTURE_DIR:." "$ROOT/target/debug/examples/runtime_c7_safety_probe" | tee "$DEBUG_LOG"
DEBUG_PASSED="$(grep -E '^Passed:' "$DEBUG_LOG" | tail -1 | awk '{print $2}')"
DEBUG_STATUS="$(grep -E '^Status:' "$DEBUG_LOG" | tail -1)"

echo ""
echo "Building C.7 probe (release)..."
cargo build --release --example runtime_c7_safety_probe >/dev/null

echo "Running C.7 probe (release)..."
RELEASE_LOG="$OUT_DIR/c7-safety-release.log"
DYLD_LIBRARY_PATH="$FIXTURE_DIR:." "$ROOT/target/release/examples/runtime_c7_safety_probe" | tee "$RELEASE_LOG"
RELEASE_PASSED="$(grep -E '^Passed:' "$RELEASE_LOG" | tail -1 | awk '{print $2}')"
RELEASE_STATUS="$(grep -E '^Status:' "$RELEASE_LOG" | tail -1)"

echo ""
echo "=== C.7 Comparative Summary ==="
...

{
  ...
  echo '  "test_type": "safety_guardrails",'
  ...
  echo "}"
} > "$OUT_DIR/c7-safety-gate.json"

{
  echo "# Phase C.7: Runtime Safety Gate Results"
  echo ""
...
} > "$OUT_DIR/c7-safety-gate.md"

if [[ "$DEBUG_PASSED" != "$RELEASE_PASSED" ]]; then
  echo "C.7 gate failed: debug/release divergence" >&2
  exit 1
fi

echo ""
echo "C.7 gate PASSED: debug and release are equivalent ($DEBUG_PASSED)."
