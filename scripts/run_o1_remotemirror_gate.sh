...
OUT_DIR="$ROOT/target/runtime-probe/o1-remotemirror"
FIXTURE_DIR="$ROOT/target/runtime-probe/resilient-fixtures"
MD_OUT="$OUT_DIR/o1-remotemirror-summary.md"
JSON_OUT="$OUT_DIR/o1-remotemirror-summary.json"
...
cargo build --example runtime_o1_remotemirror_probe >/dev/null
cargo build --release --example runtime_o1_remotemirror_probe >/dev/null

DEBUG_LOG="$OUT_DIR/o1-remotemirror-debug.log"
RELEASE_LOG="$OUT_DIR/o1-remotemirror-release.log"

DYLD_LIBRARY_PATH="$FIXTURE_DIR:." "$ROOT/target/debug/examples/runtime_o1_remotemirror_probe" | tee "$DEBUG_LOG"
DYLD_LIBRARY_PATH="$FIXTURE_DIR:." "$ROOT/target/release/examples/runtime_o1_remotemirror_probe" | tee "$RELEASE_LOG"
...
if [[ -z "$debug_passed" || -z "$release_passed" ]]; then
  echo "Missing summary output from O.1 probe" >&2
  exit 1
fi
...
cat > "$MD_OUT" <<EOF
# O.1 RemoteMirror Baseline Gate
...
if [[ "$status" != "PASS" ]]; then
  echo "O.1 RemoteMirror gate FAILED" >&2
  exit 1
fi

echo "O.1 RemoteMirror gate PASSED"
