...
OUT_DIR="$ROOT/target/runtime-probe/p3-codable"
FIXTURE_DIR="$ROOT/target/runtime-probe/resilient-fixtures"
MD_OUT="$OUT_DIR/p3-codable-summary.md"
JSON_OUT="$OUT_DIR/p3-codable-summary.json"
...
cargo build --example runtime_p3_codable_probe >/dev/null
cargo build --release --example runtime_p3_codable_probe >/dev/null

DEBUG_LOG="$OUT_DIR/p3-codable-debug.log"
RELEASE_LOG="$OUT_DIR/p3-codable-release.log"

DYLD_LIBRARY_PATH="$FIXTURE_DIR:." "$ROOT/target/debug/examples/runtime_p3_codable_probe" | tee "$DEBUG_LOG"
DYLD_LIBRARY_PATH="$FIXTURE_DIR:." "$ROOT/target/release/examples/runtime_p3_codable_probe" | tee "$RELEASE_LOG"

debug_passed="$(grep -E '^Passed:' "$DEBUG_LOG" | tail -1 | awk '{print $2}')"
release_passed="$(grep -E '^Passed:' "$RELEASE_LOG" | tail -1 | awk '{print $2}')"
debug_flags="$(grep -E '^p3 codable parity =>' "$DEBUG_LOG" | tail -1 | sed 's/^p3 codable parity => //')"
release_flags="$(grep -E '^p3 codable parity =>' "$RELEASE_LOG" | tail -1 | sed 's/^p3 codable parity => //')"

if [[ -z "$debug_passed" || -z "$release_passed" ]]; then
  echo "Missing summary output from P.3 probe" >&2
  exit 1
fi
...
cat > "$MD_OUT" <<EOF
# P.3 Codable / Serialization Gate
...
if [[ "$status" != "PASS" ]]; then
  echo "P.3 Codable gate FAILED" >&2
  exit 1
fi

echo "P.3 Codable gate PASSED"
