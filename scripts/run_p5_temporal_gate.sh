...
OUT_DIR="$ROOT/target/runtime-probe/p5-temporal"
FIXTURE_DIR="$ROOT/target/runtime-probe/resilient-fixtures"
MD_OUT="$OUT_DIR/p5-temporal-summary.md"
JSON_OUT="$OUT_DIR/p5-temporal-summary.json"
...
cargo build --example runtime_p5_temporal_probe >/dev/null
cargo build --release --example runtime_p5_temporal_probe >/dev/null

DEBUG_LOG="$OUT_DIR/p5-temporal-debug.log"
RELEASE_LOG="$OUT_DIR/p5-temporal-release.log"

DYLD_LIBRARY_PATH="$FIXTURE_DIR:." "$ROOT/target/debug/examples/runtime_p5_temporal_probe" | tee "$DEBUG_LOG"
DYLD_LIBRARY_PATH="$FIXTURE_DIR:." "$ROOT/target/release/examples/runtime_p5_temporal_probe" | tee "$RELEASE_LOG"

debug_passed="$(grep -E '^Passed:' "$DEBUG_LOG" | tail -1 | awk '{print $2}')"
release_passed="$(grep -E '^Passed:' "$RELEASE_LOG" | tail -1 | awk '{print $2}')"
debug_flags="$(grep -E '^p5 temporal parity =>' "$DEBUG_LOG" | tail -1 | sed 's/^p5 temporal parity => //')"
release_flags="$(grep -E '^p5 temporal parity =>' "$RELEASE_LOG" | tail -1 | sed 's/^p5 temporal parity => //')"

if [[ -z "$debug_passed" || -z "$release_passed" ]]; then
  echo "Missing summary output from P.5 probe" >&2
  exit 1
fi
...
if [[ "$status" != "PASS" ]]; then
  echo "P.5 Temporal gate FAILED" >&2
  exit 1
fi

echo "P.5 Temporal gate PASSED"
