...
OUT_DIR="$ROOT/target/runtime-probe/p4-collections"
FIXTURE_DIR="$ROOT/target/runtime-probe/resilient-fixtures"
MD_OUT="$OUT_DIR/p4-collections-summary.md"
JSON_OUT="$OUT_DIR/p4-collections-summary.json"
...
cargo build --example runtime_p4_collections_probe >/dev/null
cargo build --release --example runtime_p4_collections_probe >/dev/null

DEBUG_LOG="$OUT_DIR/p4-collections-debug.log"
RELEASE_LOG="$OUT_DIR/p4-collections-release.log"

DYLD_LIBRARY_PATH="$FIXTURE_DIR:." "$ROOT/target/debug/examples/runtime_p4_collections_probe" | tee "$DEBUG_LOG"
DYLD_LIBRARY_PATH="$FIXTURE_DIR:." "$ROOT/target/release/examples/runtime_p4_collections_probe" | tee "$RELEASE_LOG"

debug_passed="$(grep -E '^Passed:' "$DEBUG_LOG" | tail -1 | awk '{print $2}')"
release_passed="$(grep -E '^Passed:' "$RELEASE_LOG" | tail -1 | awk '{print $2}')"
debug_flags="$(grep -E '^p4 collections parity =>' "$DEBUG_LOG" | tail -1 | sed 's/^p4 collections parity => //')"
release_flags="$(grep -E '^p4 collections parity =>' "$RELEASE_LOG" | tail -1 | sed 's/^p4 collections parity => //')"

if [[ -z "$debug_passed" || -z "$release_passed" ]]; then
  echo "Missing summary output from P.4 probe" >&2
  exit 1
fi
...
cat > "$MD_OUT" <<EOF
# P.4 Foundation Collections Gate
...
if [[ "$status" != "PASS" ]]; then
  echo "P.4 Collections gate FAILED" >&2
  exit 1
fi

echo "P.4 Collections gate PASSED"
