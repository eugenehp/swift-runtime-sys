...
OUT_DIR="$ROOT/target/runtime-probe/p2-url"
FIXTURE_DIR="$ROOT/target/runtime-probe/resilient-fixtures"
MD_OUT="$OUT_DIR/p2-url-summary.md"
JSON_OUT="$OUT_DIR/p2-url-summary.json"
...
cargo build --example runtime_p2_url_probe >/dev/null
cargo build --release --example runtime_p2_url_probe >/dev/null

DEBUG_LOG="$OUT_DIR/p2-url-debug.log"
RELEASE_LOG="$OUT_DIR/p2-url-release.log"

DYLD_LIBRARY_PATH="$FIXTURE_DIR:." "$ROOT/target/debug/examples/runtime_p2_url_probe" | tee "$DEBUG_LOG"
DYLD_LIBRARY_PATH="$FIXTURE_DIR:." "$ROOT/target/release/examples/runtime_p2_url_probe" | tee "$RELEASE_LOG"

debug_passed="$(grep -E '^Passed:' "$DEBUG_LOG" | tail -1 | awk '{print $2}')"
release_passed="$(grep -E '^Passed:' "$RELEASE_LOG" | tail -1 | awk '{print $2}')"
debug_flags="$(grep -E '^p2 url parity =>' "$DEBUG_LOG" | tail -1 | sed 's/^p2 url parity => //')"
release_flags="$(grep -E '^p2 url parity =>' "$RELEASE_LOG" | tail -1 | sed 's/^p2 url parity => //')"

if [[ -z "$debug_passed" || -z "$release_passed" ]]; then
  echo "Missing summary output from P.2 probe" >&2
  exit 1
fi
...
if [[ "$status" != "PASS" ]]; then
  echo "P.2 URL gate FAILED" >&2
  exit 1
fi

echo "P.2 URL gate PASSED"
