...
OUT_DIR="$ROOT/target/runtime-probe/o4-packs-span"
FIXTURE_DIR="$ROOT/target/runtime-probe/resilient-fixtures"
MD_OUT="$OUT_DIR/o4-packs-span-summary.md"
JSON_OUT="$OUT_DIR/o4-packs-span-summary.json"
...
cargo build --example runtime_o4_packs_span_probe >/dev/null
cargo build --release --example runtime_o4_packs_span_probe >/dev/null

DEBUG_LOG="$OUT_DIR/o4-packs-span-debug.log"
RELEASE_LOG="$OUT_DIR/o4-packs-span-release.log"

DYLD_LIBRARY_PATH="$FIXTURE_DIR:." "$ROOT/target/debug/examples/runtime_o4_packs_span_probe" | tee "$DEBUG_LOG"
DYLD_LIBRARY_PATH="$FIXTURE_DIR:." "$ROOT/target/release/examples/runtime_o4_packs_span_probe" | tee "$RELEASE_LOG"
...
if [[ -z "$debug_passed" || -z "$release_passed" ]]; then
  echo "Missing summary output from O.4/O.5 probe" >&2
  exit 1
fi
...
cat > "$JSON_OUT" <<EOF
{
  "gate": "o4_packs_span_abi_coverage",
  "status": "$status",
...
cat > "$MD_OUT" <<EOF
# O.4/O.5 Parameter Pack & Span ABI Coverage Gate
...
if [[ "$status" != "PASS" ]]; then
  echo "O.4/O.5 packs & span gate FAILED" >&2
  exit 1
fi

echo "O.4/O.5 packs & span gate PASSED"
