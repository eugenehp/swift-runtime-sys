...
OUT_DIR="$ROOT/target/runtime-probe/o2-concurrency-abi"
FIXTURE_DIR="$ROOT/target/runtime-probe/resilient-fixtures"
MD_OUT="$OUT_DIR/o2-concurrency-abi-summary.md"
JSON_OUT="$OUT_DIR/o2-concurrency-abi-summary.json"
...
cargo build --example runtime_o2_concurrency_abi_probe >/dev/null
cargo build --release --example runtime_o2_concurrency_abi_probe >/dev/null

DEBUG_LOG="$OUT_DIR/o2-concurrency-abi-debug.log"
RELEASE_LOG="$OUT_DIR/o2-concurrency-abi-release.log"

DYLD_LIBRARY_PATH="$FIXTURE_DIR:." "$ROOT/target/debug/examples/runtime_o2_concurrency_abi_probe" | tee "$DEBUG_LOG"
DYLD_LIBRARY_PATH="$FIXTURE_DIR:." "$ROOT/target/release/examples/runtime_o2_concurrency_abi_probe" | tee "$RELEASE_LOG"
...
if [[ -z "$debug_passed" || -z "$release_passed" ]]; then
  echo "Missing summary output from O.2 probe" >&2
  exit 1
fi
...
cat > "$JSON_OUT" <<EOF
{
  "gate": "o2_concurrency_abi_control_path",
  "status": "$status",
...
cat > "$MD_OUT" <<EOF
# O.2 Concurrency ABI Control-Path Gate
...
if [[ "$status" != "PASS" ]]; then
  echo "O.2 concurrency ABI gate FAILED" >&2
  exit 1
fi

echo "O.2 concurrency ABI gate PASSED"
